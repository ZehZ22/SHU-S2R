...
def waveforce_irregular(t, L, h, T, beta_r, w, fai, U):
    ...
    # 采用吃水*船宽代替L*L
    # tau_wave[0] = alpha * fwx / (0.5 * 1000 * U ** 2 * 180)
    # tau_wave[1] = alpha * fwy / (0.5 * 1000 * U ** 2 * 180)
    # tau_wave[2] = alpha * fwn / (0.5 * 1000 * U ** 2 * L * 180)

    tau_wave[0] = alpha * fwx / (0.5 * 1000 * U ** 2 * L ** 2)
    tau_wave[1] = alpha * fwy / (0.5 * 1000 * U ** 2 * L ** 2)
    tau_wave[2] = alpha * fwn / (0.5 * 1000 * U ** 2 * L ** 3)
