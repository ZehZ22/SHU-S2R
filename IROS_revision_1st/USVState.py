...
class USVState:
    def __init__(self, waypoints, current_index, x, ui, model_func=marinerwind, wind_speed=4.0, wind_direction=60.0,
                 ...
                 wave_direction=np.radians(70), wave_height=4.0,
        ...
        # 随机化参数
        if randomize_params:
            self.wind_speed = self.randomize_value(1.0, 5.0, no_seed_random)  
            self.wind_direction = np.radians(self.randomize_value(0.0, 90.0, no_seed_random))
            self.current_speed = self.randomize_value(0.5, 2.0, no_seed_random) 
            self.current_direction = self.randomize_value(0.0, 90.0, no_seed_random)
            self.wave_height = self.randomize_value(2.0, 5.0, no_seed_random)  
            self.wave_direction = np.radians(self.randomize_value(0.0, 90.0, no_seed_random))  
    ...
    def step(self, ui):
        ...
        w = np.random.uniform(0.1, 1.5, 30)
        fai = np.random.uniform(0, 2 * np.pi, 30)
