...
def KCS_ode(t, v, delta_c, ext_force=None, ext_ctx=None, current_func=None, current_ctx=None):
    ...
    # ----------------------------------------------------
    # External Disturbance Forces (optional)
    # ----------------------------------------------------
    Xp_W = 0.0
    Yp_W = 0.0
    Np_W = 0.0
    if callable(ext_force):
        dX, dY, dN = ext_force(t, v, ext_ctx)
        Xp_W += float(dX)
        Yp_W += float(dY)
        Np_W += float(dN)
